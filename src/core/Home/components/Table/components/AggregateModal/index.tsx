...
import {ModalWrapper, StyledTableCell, ModalHeadWrapper, StyledTableHead, StyledTableHeadCell} from './styles'
...
const getAggregate = (selectedPlayers: Player[]) => {
  ...
  selectedPlayers.forEach((player) => {
    ...
    const playerPositions = positions.map(position => position.name).join(', ')
    
    aggregate.total.base = Number(aggregate.total.base) + Number(playerSalary)
    aggregate.total.guaranteed = Number(aggregate.total.guaranteed) +  Number(playerCompensation)

    aggregate.average = {
      base: Math.round(Number(aggregate.total.base) / selectedPlayers.length),
      guaranteed: Math.round(Number(aggregate.total.guaranteed) / selectedPlayers.length)
    }
...
const CustomRow = ({rowName, row}: {rowName: string, row: AggregateRow}) => {
  return <TableRow
    tabIndex={-1}
    key={1}
  >
    <StyledTableCell>
      <b>{rowName} {rowName === 'total' ? 'Salaries' : 'Salary' }</b>
    </StyledTableCell>
    <TableCell>
      {currencyFormatter.format(Number(row.base))}{' '}{row.basePlayerName && `(${row.basePlayerName} ${row.basePlayerPositions && `- ${row.basePlayerPositions}`})`}
    </TableCell>
    <TableCell>
      {currencyFormatter.format(Number(row.guaranteed))}{' '}{row.guaranteedPlayerName && `(${row.guaranteedPlayerName} ${row.guaranteedPlayerPositions && `- ${row.guaranteedPlayerPositions}`})`}
    </TableCell>
    
  </TableRow>
}

export const AggregateModal = ({showAggregate, selected, players, closeModal}: AggregateModalProps) => {
  const selectedPlayers = useMemo(() => players.filter(player => selected.includes(player.id)), [players, selected])
  const aggregate = useMemo(() => getAggregate(selectedPlayers), [selectedPlayers])

  return <Modal open={showAggregate} BackdropProps={{onClick: closeModal}}>
    <ModalWrapper>
      <ModalHeadWrapper>
        <h2>Aggregate ({selectedPlayers.length} player{selectedPlayers.length > 1 && 's'})</h2>
          ...
          <Table>
            <StyledTableHead>
              <TableRow>
                ...
                <StyledTableHeadCell>Guaranteed Compensation</StyledTableHeadCell>
              </TableRow>
            </StyledTableHead>
            <TableBody>
