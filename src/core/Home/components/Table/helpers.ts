...
export const getPlayerUrl = (firstName: string, lastName: string) => {
  ...
  return `
    https://www.mlssoccer.com/players/${fullName
      .toLowerCase()
      .replace(" ", "-")}
  `;
};
...
export const playerHasSearchValue = (
  player: Player,
  value: string
): boolean => {
  const {
  ...
  const matches = search_values.filter((val) =>
    searchString.toLowerCase().includes(val.toLowerCase())
  ).length;
